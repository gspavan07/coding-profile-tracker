  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-[#00000063]">
      <div className="bg-gray-800 flex flex-col justify-center items-center text-white p-6 rounded-lg min-w-96 max-w-3xl">
        ...
        <p>
          Rank: <span className="font-bold">{selectedStudent.rank || "N/A"}</span>
        </p>
        <p>
          {selectedStudent.roll || "N/A"} ({selectedStudent.branch || "N/A"})
        </p>
        ...
        <div className="bg-gray-700 p-4 rounded-lg text-center w-full mb-4">
          ...
          <p>{selectedStudent.badges?.length > 0 ? selectedStudent.badges.join(", ") : "No Badges"}</p>
        </div>
        ...
        <div className="bg-gray-700 p-4 rounded-lg text-center w-full mb-4">
          ...
          {selectedStudent.languages?.length > 0 ? (
            selectedStudent.languages.map((lang, index) => (
              <p key={index}>
                {lang.name}: {lang.solved} problems
              </p>
            ))
          ) : (
            <p>No Data</p>
